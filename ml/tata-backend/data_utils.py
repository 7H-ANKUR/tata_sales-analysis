...
def build_forecast_curve(
    ...
    Uses GBR revenue model + computes derived profit estimate.
    """
    bundle  = load_bundle()
    ...
    for yr in range(start_year, end_year + 1):
        ...
        row = {
            ...
            "local_manufacturing_cost_usd": 15500 * g,
            "marketing_expense_usd": 5000 * 1.02 ** (yr - 2024),
            "r_and_d_expense_usd": 8500 * 1.05 ** (yr - 2024),
            "dealer_commission_pct": 6.0,
            "rev_estimate": rev_est,
            "total_revenue": rev_est,
            "operating_expense_usd": 17000 * g,
            "warranty_cost_usd": 1300 * g,
            "capital_expenditure_usd": 32000 * g,
        }
        eng = engineer_features(row)
        X   = pd.DataFrame([[eng[f] for f in feats]], columns=feats)
        pred_rev = float(gbr.predict(X)[0])
        pred_profit = pred_rev * 0.175  # ~17.5% avg margin
